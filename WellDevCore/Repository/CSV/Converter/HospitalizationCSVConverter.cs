...
namespace bolnica.Repository.CSV.Converter
{
    public class HospitalizationCSVConverter : ICSVConverter<Hospitalization>
    {
        ...
        public Hospitalization ConvertCSVFormatToEntity(string entityCSVFormat)
        {
            ...
            Hospitalization hospitalization = new Hospitalization(); // Maja migracija
                /*long.Parse(tokens[0]), (User) new Patient(long.Parse(tokens[1])), new Doctor(long.Parse(tokens[2])), new Period(DateTime.Parse(tokens[3]), DateTime.Parse(tokens[4])), new Room(long.Parse(tokens[5])));*/
