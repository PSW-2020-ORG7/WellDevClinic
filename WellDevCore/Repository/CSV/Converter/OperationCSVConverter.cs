...
namespace bolnica.Repository
{
    public class OperationCSVConverter : ICSVConverter<Operation>
    {
        ...
        { 
            ...
            Operation operation = new Operation(//); // Maja migracija
                long.Parse(tokens[0]), (User)new Patient(long.Parse(tokens[1])), new Doctor(long.Parse(tokens[2])), tokens[3], new Period(DateTime.Parse(tokens[4]), DateTime.Parse(tokens[5])), new Room(long.Parse(tokens[6])));
            return operation;
        }
